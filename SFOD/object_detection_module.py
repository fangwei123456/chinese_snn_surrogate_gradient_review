...
class DetectionLitModule(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.fusion:
            self.neck = DetectionNeck(self.backbone.out_channels, args.fusion_layers, T=args.T, norm=args.norm, sn=args.sn)  # Neck network
            self.pyramid_ext = DetectionPyramid(self.neck.out_channel, mode=args.mode, T=args.T, norm=args.norm, sn=args.sn)  # pyramid extraction network
    ...
    def step(self, batch, batch_idx, mode):
        ...
        if self.fusion:
            ...
            spikingjelly.activation_based.functional.reset_net(self.pyramid_ext)
