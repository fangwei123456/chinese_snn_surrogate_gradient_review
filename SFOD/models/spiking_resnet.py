...
class FPNvv(nn.Module):
    def __init__(self, stage_dims,neuron: callable = None, **kwargs):
        super(FPNvv, self).__init__()
        self.inplanes = 64
        ...
        #     nn.Conv2d(stage_dims[5], stage_dims[2], kernel_size=1, stride=1, padding=0),  # Reduce channels
        #     nn.BatchNorm2d(stage_dims[2])
        # )
        # self.top_lif = neuron(**kwargs)
       
        # # Lateral layers
        # self.latlayer5 = nn.Sequential(
        #     nn.Conv2d(stage_dims[4], stage_dims[2], kernel_size=1, stride=1, padding=0),
        #     nn.BatchNorm2d(stage_dims[2])
        # )
        # self.lat_lif5 = neuron(**kwargs)

        self.latlayer4 = nn.Sequential(
            nn.Conv2d( stage_dims[3], stage_dims[2], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(stage_dims[2])
        )
        self.lat_lif4 = neuron(**kwargs)

        self.latlayer3 = nn.Sequential(
            nn.Conv2d( stage_dims[2], stage_dims[2], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(stage_dims[2])
        )
        self.lat_lif3 = neuron(**kwargs)

        self.latlayer2 = nn.Sequential(
            nn.Conv2d( stage_dims[1], stage_dims[2], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(stage_dims[2])
        )
        self.lat_lif2 = neuron(**kwargs)

        self.latlayer1 = nn.Sequential(
            nn.Conv2d( stage_dims[0], stage_dims[2], kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(stage_dims[2])
        )
        self.lat_lif1 = neuron(**kwargs)
        ...
        self.lif4 = neuron(**kwargs)

        # self.smooth5 = nn.Sequential(
        #     nn.Conv2d(stage_dims[2], stage_dims[2], kernel_size=3, stride=1, padding=1),
        #     nn.BatchNorm2d(stage_dims[2])
        # )
        # self.lif5 = neuron(**kwargs)

        # self.smooth6 = nn.Sequential(
        #     nn.Conv2d(stage_dims[2], stage_dims[2], kernel_size=3, stride=1, padding=1),
        #     nn.BatchNorm2d(stage_dims[2])
        # )
        # self.lif6 = neuron(**kwargs)
    ...
    def forward(self, c1,c2,c3,c4,c5,c6):
        # Top-down
        # T6,B6,C6,H6,W6 = c6.shape
        # p6=self.top_lif(c6)
        # p6 = p6.flatten(0,1)#T*N,C,H,W
        # p6 = self.toplayer(p6)
        # p6=p6.reshape(T6,B6,self.stage_dims[2],H6,W6).contiguous()
        ...
        # c5=c5.reshape(T5,B5,self.stage_dims[2],H5,W5).contiguous()
        
        # c5=c5.flatten(0,1)
        # p5 = self._upsample_add(p6, c5)
        ...
        T1,B1,C1,H1,W1 = c1.shape
        c1=self.lat_lif1(c1)
        c1 = c1.flatten(0,1)#T*N,C,H,W
        c1=self.latlayer1(c1)
        c1=c1.reshape(T1,B1,self.stage_dims[2],H1,W1).contiguous()
        
        c1=c1.flatten(0,1)
        p1 = self._upsample_add(p2, c1)

        # Smooth
        # _,C6_,H6_,W6_=p6.shape
        # p6=p6.reshape(T6,B6,C6_,H6_,W6_).contiguous()
        # p6 = self.smooth6(p6.flatten(0,1)).reshape(T6,B6,C6_,H6_,W6_).contiguous()

        # _,C5_,H5_,W5_=p5.shape
        # p5=p5.reshape(T5,B5,C5_,H5_,W5_).contiguous()
        # p5=self.lif5(p5)
        # p5 = self.smooth5(p5.flatten(0,1)).reshape(T5,B5,C5_,H5_,W5_).contiguous()

        _,C4_,H4_,W4_=p4.shape
        p4=p4.reshape(T4,B4,C4_,H4_,W4_).contiguous()
        p4=self.lif4(p4)
        p4 = self.smooth4(p4.flatten(0,1)).reshape(T4,B4,C4_,H4_,W4_).contiguous()
       
        _,C3_,H3_,W3_=p3.shape
        p3=p3.reshape(T3,B3,C3_,H3_,W3_).contiguous()
        p3=self.lif3(p3)
        p3 = self.smooth3(p3.flatten(0,1)).reshape(T3,B3,C3_,H3_,W3_).contiguous()
        

        _,C2_,H2_,W2_=p2.shape
        p2=p2.reshape(T2,B2,C2_,H2_,W2_).contiguous()
        p2=self.lif2(p2)
        p2 = self.smooth2(p2.flatten(0,1)).reshape(T2,B2,C2_,H2_,W2_).contiguous()

        _,C1_,H1_,W1_=p1.shape
        p1=p1.reshape(T1,B1,C1_,H1_,W1_).contiguous()
        p1=self.lif1(p1)
        p1 = self.smooth1(p1.flatten(0,1)).reshape(T1,B1,C1_,H1_,W1_).contiguous()
        

        return p1,p2, p3, p4,c5,c6
...
class SpikingResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, single_step_neuron: callable = None, **kwargs):
        ...
        #         nn.Conv2d(num_features, num_classes,kernel_size=1, bias=bias), ),
        #     neuron(**kwargs)
        # )
    ...
    def _forward_impl(self, x):
        ...
        x = self.fc(x)
        # out=[x1,x2,x3,x4]
        # out=out.permute(1, 0, 2,3,4)#N,T,C,H,W
...
class MultiStepSpikingResNet(nn.Module):
    ...
    def _forward_impl(self, x: torch.Tensor,classify=True):
        # See note [TorchScript super()]
        x_seq = None
        if x.dim() == 5:
            # x.shape = [T, N, C, H, W]
            x_seq = functional.seq_to_ann_forward(x, [self.conv1_1, self.bn1])
        else:
        ...
        if classify:
            ...
            x_seq = torch.flatten(x_seq, 2)
            # x_seq = self.fc(x_seq.mean(0))
            x_seq = functional.seq_to_ann_forward(x_seq, self.fc1)
            return x_seq
        else:
            ...
            x_seq = self.features1(x_seq)
            output = self.backb(x_seq )
            return output
        ##################################################################################
        return x_seq
    ...
    def forward(self, x,classify=True):
        ...
        :rtype: torch.Tensor
        """
        return self._forward_impl(x,classify=True)
...
def _multi_step_spiking_resnet(arch, block, layers, pretrained, progress, T, multi_step_neuron, **kwargs):
    ...
    # if pretrained:
    #     model.load_state_dict(model_zoo.load_url(model_urls[arch], model_dir='.'), strict=False) 
    return model
