...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, patch_size=4, in_c=2, embed_dim=96, norm_layer=None):
        super().__init__()
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        #x_ = x.flatten(0, 1)
        x = self.fc1_linear(x)
        x = self.fc1_bn(x.transpose(-1, -2)).transpose(-1, -2).reshape(T, -1, N, self.c_hidden).contiguous()
        x = self.fc1_lif(x)

        x = self.fc2_linear(x.flatten(0,1))
        x = self.fc2_bn(x.transpose(-1, -2)).transpose(-1, -2).reshape(T, -1, N, C).contiguous()
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        B_, N, C = x.shape
        x_for_qkv = x.permute(0,2,1)#.flatten(0, 1)
        q_conv_out = self.q_conv(x_for_qkv)
        q_conv_out = self.q_bn(q_conv_out).reshape(T,-1,C,N).contiguous()
        q_conv_out = self.q_lif(q_conv_out)
        q = q_conv_out.transpose(-1, -2).reshape(T, -1, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()#(T,B,self.num_heads,N,C//self.num_heads)
        q=q.flatten(0,1)#(B_,self.num_heads,N,C//self.num_heads)

        k_conv_out = self.k_conv(x_for_qkv)
        k_conv_out = self.k_bn(k_conv_out).reshape(T,-1,C,N).contiguous()
        k_conv_out = self.k_lif(k_conv_out)
        k = k_conv_out.transpose(-1, -2).reshape(T, -1, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()
        k=k.flatten(0,1)

        v_conv_out = self.v_conv(x_for_qkv)
        v_conv_out = self.v_bn(v_conv_out).reshape(T,-1,C,N).contiguous()
        v_conv_out = self.v_lif(v_conv_out)
        v = v_conv_out.transpose(-1, -2).reshape(T, -1, N, self.num_heads, C//self.num_heads).permute(0, 1, 3, 2, 4).contiguous()
        v=v.flatten(0,1)
        ...
        # reshape: -> [batch_size*num_windows, Mh*Mw, total_embed_dim](B_,self.num_heads,N,C//self.num_heads)->(B_,N,self.num_heads,C//self.num_heads)
        x = (attn @ v).transpose(1, 2).reshape(B_, N, C)

        # x = self.proj(x)
        # x = self.proj_drop(x)

        x = x.permute(0,2,1).reshape(T, -1, C, N).contiguous()
        x = self.attn_lif(x)
        x = x.flatten(0,1)#(B_,C,N)
        x = self.proj_lif(self.proj_bn(self.proj_conv(x)).reshape(T,-1,C,N))
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        x=x.flatten(0,1)
        # x: [B, L, C]
        x, H_, W_ = self.patch_embed(x)  # 对图像下采样4倍
        x = self.pos_drop(x)

        # 依次传入各个stage中
        outputs=[]
        for layer in self.layers:
            x, H, W,x_ori = layer(x, H_, W_)
            out=x_ori.permute(0,2,1).reshape(T,B,-1,H_,W_)
