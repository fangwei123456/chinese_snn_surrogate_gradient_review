...
class DetectionPyramid(nn.Module):
    def __init__(self, in_channel, T, mode='norm', num_layers=2, growth_rate=24, norm:str='bn', sn:str='lif'):
        """
        ...
        if mode == 'norm':
            self.pyramid_ext = nn.ModuleList(
                [
                    nn.Sequential(
                        SpikingBlock(in_channel, in_channel // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SpikingBlock(in_channel // 2, self.out_channels[0], kernel_size=3, padding=1, stride=1, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[0], self.out_channels[0] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[0] // 2, self.out_channels[1], kernel_size=3, padding=1,
                                     stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[1], self.out_channels[1] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[1] // 2, self.out_channels[2], kernel_size=3, padding=1,
                                     stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[2], self.out_channels[2] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[2] // 2, self.out_channels[3], kernel_size=3, padding=1,
                                     stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[3], self.out_channels[3] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[3] // 2, self.out_channels[4], kernel_size=3, padding=1,
                                     stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[4], self.out_channels[4] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                ...
                [
                    nn.Sequential(
                        ...
                        SpikingBlock(in_channel // 2 + num_layers * growth_rate, self.out_channels[0], kernel_size=3,
                                     padding=1, stride=1, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[0], self.out_channels[0] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        _DenseBlock(
                        ...
                        SpikingBlock(self.out_channels[0] // 2 + num_layers * growth_rate, self.out_channels[1],
                                     kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[1], self.out_channels[1] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        _DenseBlock(
            ...
            self.pyramid_ext = nn.ModuleList(
                [
                    nn.Sequential(
                        SpikingBlock(in_channel, in_channel // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(in_channel // 2, in_channel // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(in_channel // 2, self.out_channels[0], kernel_size=3, padding=1, stride=1, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[0], self.out_channels[0] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(self.out_channels[0] // 2, self.out_channels[0] // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[0] // 2, self.out_channels[1], kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[1], self.out_channels[1] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(self.out_channels[1] // 2, self.out_channels[1] // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[1] // 2, self.out_channels[2], kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[2], self.out_channels[2] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(self.out_channels[2] // 2, self.out_channels[2] // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[2] // 2, self.out_channels[3], kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[3], self.out_channels[3] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(self.out_channels[3] // 2, self.out_channels[3] // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[3] // 2, self.out_channels[4], kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn),
                    ),
                    nn.Sequential(
                        SpikingBlock(self.out_channels[4], self.out_channels[4] // 2, kernel_size=1, T=T, norm=norm, sn=sn),
                        SEWBlock(self.out_channels[4] // 2, self.out_channels[4] // 4, T=T, norm=norm, sn=sn),
                        SpikingBlock(self.out_channels[4] // 2, self.out_channels[5], kernel_size=3, padding=1, stride=2, T=T, norm=norm, sn=sn)
                    )
                ]
            )
        elif mode == 'skip':
