...
def create_neuron(neu: str, T:int=5, **kwargs):
    if neu == 'if':
        return neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='cupy')
    elif neu == 'lif':
        return neuron.LIFNode(tau=2., surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='cupy')
    elif neu == 'tandem':
        return TandemIF()
    elif neu == 'relu':
        return nn.ReLU()
        ...
        return layer.MultiStepContainer(OnlineLIFNode(T, tau=2., surrogate_function=surrogate.ATan(), detach_reset=True))
    else:
        raise NotImplementedError


def create_norm(norm: str, channels, T:int=5):
    if norm == 'bn':
        return layer.SeqToANNContainer(nn.BatchNorm2d(channels))
    elif norm == 'tebn':
        return TEBN(T, channels)
    elif norm == 'osr':
