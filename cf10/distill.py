...
def main():
    ...
    parser.add_argument('-prefix', default='distill', type=str)

    parser.add_argument('-ann', default='/home/wfang/chinese_review/cf10/logs/pt/logs/ann_e256_b128_sgd_lr0.1_c128_amp/checkpoint_latest.pth', type=str)
    ...
    out_dir = f'{args.prefix}_a{args.alpha}_t{args.temp}_b{args.beta}_T_{args.T}_e{args.epochs}_b{args.b}_{args.opt}_lr{args.lr}_c{args.channels}'
    if args.amp:
        out_dir += '_amp'
    ...
    ann = baseline_ann.CIFAR10Net(args.channels)
    ann.to(args.device)
    ann.load_state_dict(torch.load(args.ann, map_location=args.device)['net'])
    ann.eval()
        ...
        for img, label in train_data_loader:
            ...
            with torch.cuda.amp.autocast(enabled=scaler is not None):
                ...
                if args.alpha > 0:
                    ...
                    loss_response = F.cross_entropy(y / args.temp, y_ann / args.temp)

                    loss = (args.alpha * args.temp * args.temp) * loss_response + (1. - args.alpha) * loss
                    ...
                    loss = F.cross_entropy(y, label, label_smoothing=0.1)
                    loss_dis = 0.
                    for a, s in zip(features, features_s):
                        loss_dis = loss_dis + F.mse_loss(a, s.mean(0))

                    loss = loss + args.beta * loss_dis
