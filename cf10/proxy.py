    ...
    def __init__(self, channels):
        ...
        for i in range(2):
            for j in range(3):
                ...
                conv.append(neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='cupy', v_reset=None))
        ...
        self.fc = nn.Sequential(
            ...
            neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='cupy', v_reset=None),
...
def main():
    ...
    parser.add_argument('-prefix', default='proxy', type=str)
    ...
    snn = CIFAR10Net(args.channels)
    ann = CIFAR10ANN(args.channels)
    ...
    if args.T == 1:
        functional.set_backend(snn, 'torch')
    ...
    if args.opt == 'sgd':
        ...
        optimizer = torch.optim.AdamW(ann.parameters(), lr=args.lr, weight_decay=0.)
    else:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for img, label in train_data_loader:
            ...
            with torch.cuda.amp.autocast(enabled=scaler is not None):
                ...
                with torch.no_grad():
                    snn_outputs = snn(img).mean(0)
                y.data.copy_(snn_outputs)
        ...
        with torch.no_grad():
            for img, label in test_data_loader:
                ...
                loss = F.mse_loss(y, F.one_hot(label, 10).float())
