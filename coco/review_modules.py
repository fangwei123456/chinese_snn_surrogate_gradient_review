...
class CLIFSpike(nn.Module):
    def __init__(self, tau: float=2.):
        ...
        self.spike_func = surrogate.ATan()
...
class BALIFWrapper(nn.Module):
    def __init__(self, t_len, t_latency):
        # BlockALIF需要输入的数量，用此包装器先运行一次，得到数量后再新建
        super(BALIFWrapper, self).__init__()
        self.block_alif = None
        self.t_len = t_len
        self.t_latency = t_latency

    def forward(self, x):
        if self.block_alif is None:
            n = x[0][0].numel()
            print('number of neurons=', n)
            self.block_alif = DynamicReshapeModule(Blocks(n_in=n, n_out=n, t_len=self.t_len, t_latency=self.t_latency, recurrent=False,beta_grad=True,adapt=True,init_beta=0.99,init_p=0.99))
            return x
        else:
            return self.block_alif(x)


class OSRMultiStepContainer(layer.MultiStepContainer):
    def forward(self, x_seq: Tensor):
        assert isinstance(self[0], (OnlineLIFNode, OSR))
        self[0].init = True
        return super().forward(x_seq)

def create_neuron(neu: str, T:int=5, **kwargs):
    if neu == 'if':
        return neuron.IFNode(surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='torch')
    elif neu == 'lif':
        return neuron.LIFNode(tau=2., surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m', backend='torch')
    elif neu == 'tandem':
        return TandemIF()
    elif neu == 'relu':
        return nn.ReLU()
        ...
        return PSN(T=T, surrogate_function=surrogate.ATan())
    elif neu == 'spsn2':
        return SlidingPSN(k=2, surrogate_function=surrogate.ATan(), step_mode='m')
    elif neu == 'spsn3':
        return SlidingPSN(k=3, surrogate_function=surrogate.ATan(), step_mode='m')
    elif neu == 'blockalif':
        return BALIFWrapper(t_len=T, t_latency=1)
        ...
        return OSRMultiStepContainer(OnlineLIFNode(T, tau=2., surrogate_function=surrogate.ATan(), detach_reset=True))
    else:
        raise NotImplementedError


def create_norm(norm: str, channels, T:int=5, weight_init:float=1.):
    if norm == 'bn':
        bn = nn.BatchNorm2d(channels)
        if bn.affine:
            nn.init.constant_(bn.weight, weight_init)

        bn = layer.SeqToANNContainer(bn)
    elif norm == 'tebn':
        bn = TEBN(T, channels)
        if bn.bn.affine:
            nn.init.constant_(bn.bn.weight, weight_init)
    elif norm == 'osr':
        bn = OSR(T, channels)
        nn.init.constant_(bn.gamma, weight_init)
        bn = OSRMultiStepContainer(bn)
