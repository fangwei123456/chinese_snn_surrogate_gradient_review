...
'''
python train.py -sn lif -norm bn --data ./data/coco.yaml
'''
...
def train(norm, sn, hyp,  # path/to/hyp.yaml or hyp dictionary
          opt,
          device,
          callbacks
          ):
    ...
    if pretrained:#false不执行这里
        ...
        model = Model(norm, sn, cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
        ...
        model = Model(norm, sn, cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create

    for m in model.modules():
        if hasattr(m, 'backend') and m.backend == 'torch':
            m.backend = 'cupy'
    ...
    for v in model.modules():
        if hasattr(v, 'bias') and isinstance(v.bias, nn.Parameter):  # bias
            g2.append(v.bias)
        if isinstance(v, (nn.BatchNorm3d, nn.BatchNorm2d)):  # weight (no decay)
            g0.append(v.weight)
        elif hasattr(v, 'weight') and isinstance(v.weight, nn.Parameter):  # weight (with decay)
            g1.append(v.weight)

        if isinstance(v, review_modules.OSR):
            g2.append(v.beta)
            g0.append(v.gamma)
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    ...
                    energy.set_input_shape(model, torch.zeros([1, 3, 640, 640], device=device))
                    energy.set_flops(model)
                    energy.set_record_spike(model)
                    ...
                    first_flops, sops = energy.get_sops(model)
                    print('flops=', first_flops / 1e6, 'sop=', sops / 1e6)
                    if is_coco and not opt.test_only:
...
def main(norm, sn, opt, callbacks=Callbacks()):
    # Checks
    if RANK in [-1, 0]:
    ...
    if not opt.evolve:
        ...
        train(norm, sn, opt.hyp, opt, device, callbacks)
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve，演变进化
            ...
            # Train mutation
            results = train(norm, sn, hyp.copy(), opt, device, callbacks)
...
'''
g1 python train.py --sn lif --norm bn --device 0 --epochs 100 &> lif_bn.txt
g1 python train.py --sn if --norm bn --device 1 --epochs 100 &> if_bn.txt
g2 python train.py --sn clif --norm bn --device 3 --epochs 100 &> clif_bn.txt
g1 python train.py --sn spsn3 --norm bn --device 2 --epochs 100 &> spsn3_bn.txt

g2 python train.py --sn lif --norm tebn --device 0 --epochs 100
g2 python train.py --sn osr --norm osr --device 1 --epochs 100
g2 python train.py --sn blockalif --norm bn --device 2 --epochs 100
'''