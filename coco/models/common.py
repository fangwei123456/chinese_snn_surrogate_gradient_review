...
class Conv_1(nn.Module):
    # Standard convolution
    def __init__(self, norm:str, c1, c2, k, s, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = Snn_Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        # self.bn = batch_norm_2d(c2)
        self.bn = review_modules.create_norm(norm, c2, time_window, batch_norm_2d_weight_init)
...
class Snn_Conv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        y = F.conv2d(y, self.weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        y_shape.extend(y.shape[1:])
        return y.view(y_shape)
...
batch_norm_2d_weight_init = thresh
...
batch_norm_2d1_weight_init = 0.2 * thresh
...
class Pools(nn.Module):
    ...
    def forward(self,input):
        return seq_to_ann_forward(input, self.pool)
...
class zeropad(nn.Module):
    ...
    def forward(self,input):
        return seq_to_ann_forward(input, self.pad)
...
class Sample(nn.Module):
    ...
    def forward(self,input):
        return seq_to_ann_forward(input, self.up)
...
class BasicBlock_1(nn.Module):#
    def __init__(self, norm:str, sn:str, in_channels, out_channels, stride=1,e=0.5):
        super().__init__()
        # c_ = int(out_channels * e)  # hidden channels  
        c_=1024
        self.residual_function = nn.Sequential(
            # mem_update(act=False),
            review_modules.create_neuron(sn, time_window),
            Snn_Conv2d(in_channels, c_, kernel_size=3, stride=stride, padding=1, bias=False),
            # batch_norm_2d(c_),
            review_modules.create_norm(norm, c_, time_window, batch_norm_2d_weight_init),
            # mem_update(act=False),
            review_modules.create_neuron(sn, time_window),
            Snn_Conv2d(c_, out_channels, kernel_size=3, padding=1, bias=False),
            # batch_norm_2d1(out_channels),
            review_modules.create_norm(norm, out_channels, time_window, batch_norm_2d1_weight_init),
            )
        # shortcut
        self.shortcut = nn.Sequential(
            )
        if stride != 1 or in_channels != out_channels:
            self.shortcut = nn.Sequential(
                ...
                # batch_norm_2d(out_channels),
                review_modules.create_norm(norm, out_channels, time_window, batch_norm_2d_weight_init)
            )
...
class BasicBlock_2(nn.Module):
    def __init__(self, norm:str, sn:str, in_channels, out_channels,k_size=3,stride=1):
        super().__init__()
        p=None
        if k_size==3:
            pad=1
...
class Concat_res2(nn.Module):#
    def __init__(self, norm:str, sn:str, in_channels, out_channels,k_size=3, stride=1,e=0.5):
        super().__init__()
        c_ = int(out_channels * e)  # hidden channels  
        if k_size==3:
            pad=1
        ...
        if in_channels<out_channels:
            self.shortcut = nn.Sequential(                 
                ...
                review_modules.create_norm(norm, out_channels-in_channels, time_window, batch_norm_2d_weight_init),
            )
