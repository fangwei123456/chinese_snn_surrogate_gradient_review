...
class Model(nn.Module):
    def __init__(self, norm, sn, cfg='yolov3.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        super().__init__()
        ...
        self.model, self.save = parse_model(norm, sn, deepcopy(self.yaml), ch=[ch])  # model, savelist
        self.names = [str(i) for i in range(self.yaml['nc'])]  # default names
        self.inplace = self.yaml.get('inplace', True)

        if sn == 'blockalif':
            for m in self.modules():
                if isinstance(m, review_modules.BALIFWrapper):
                    m.block_alif = nn.Identity()
        ...
        if isinstance(m, Detect):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in self(torch.zeros(1, ch, s, s))])  # forward
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        # modified
        # ----------------
        input = x.unsqueeze(0).repeat(time_window, 1, 1, 1, 1)  # [T, N, C, H, W]
...
def parse_model(norm, sn, d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv,Conv_2,
            ...
            if m in (BasicBlock_2, Concat_res2, BasicBlock_1):
                args = [norm, sn] + args
            elif m in (Conv_1, ):
                args = [norm, ] + args
