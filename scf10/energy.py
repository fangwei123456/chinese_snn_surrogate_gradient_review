...
def get_sops_over_test_set(net: nn.Module, test_data_loader, args):
    ...
    with torch.no_grad():
        ...
        print(f'flops={net.conv[0].flops * 32 / 1e6:.3f}, sops={get_sops(net) * 32 / 1e6:.3f}')
...
def get_sops_over_test_set_online(net: nn.Module, test_data_loader, args):
    ...
    with torch.no_grad():
        ...
        for img, label in test_data_loader:
            img = img.to(args.device).permute(3, 0, 1, 2)
            label = label.to(args.device)
            net.set_init()
            set_input_shape(net, img[0, 0:1])  # 获取每一层输入输出的shape
            set_flops(net)  # 根据shape计算flop
            functional.reset_net(net)
            break
