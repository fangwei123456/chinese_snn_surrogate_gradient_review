                    ...
                    if type == 'blockalif':
                        conv.append(DynamicReshapeModule(
                            Blocks(n_in=channels * 32, n_out=channels * 32, t_len=T, t_latency=latency, recurrent=False,
                                   beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99)))
                    elif type == 'alif':
                        conv.append(DynamicReshapeModule(
                        ...
                        conv.append(DynamicReshapeModule(
                            LIF(n_in=channels * 32, n_out=channels * 32, t_len=T, t_latency=latency, recurrent=False,
                                beta_grad=True, adapt=False, init_beta=0.99, init_p=0.99)))
                elif i == 1:
                    # 添加Blocks实例，设置参数
                    if type == 'blockalif':
                        conv.append(DynamicReshapeModule(
...
def main():
    ...
    '''
    python blockalif_scf10.py -data-dir /datasets/CIFAR10 -amp -opt sgd -channels 128 -epochs 256 -latency 1 -out-dir ./templog
