...
class LIF(nn.Module):
    def __init__(self, n_in, n_out, t_len, t_latency, recurrent=False, beta_grad=True, adapt=False, init_beta=1,
                 init_p=1, detach_spike_grad=True, surr_grad=FastSigmoid.apply):
...
class DynamicReshapeModule(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.dim() == 4:
            C, H = original_shape[2], original_shape[3]
            N = C * H
        ...
        if len(original_shape) == 4:
            x_output = x_processed.view(T, B, C, H)
...
class CIFAR10Net(nn.Module):
    def __init__(self, channels, T, latency=1, type='blockalif'):
        super().__init__()
        conv = []
        for i in range(2):
            for j in range(3):
                ...
                if i == 0:
                    # 添加Blocks实例，设置参数
                    if type == 'blockalif':
                        conv.append(DynamicReshapeModule(
                            Blocks(n_in=channels * 32, n_out=channels * 32, t_len=T, t_latency=latency, recurrent=False,
                                   beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
                    elif type == 'alif':
                        conv.append(DynamicReshapeModule(
                            ALIF(n_in=channels * 32, n_out=channels * 32, t_len=T, t_latency=latency, recurrent=False,
                                 beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
                    elif type == 'lif':
                        conv.append(neuron.LIFNode(tau=2., surrogate_function=surrogate.ATan(), detach_reset=True,
                                                   step_mode='m', backend='cupy'))
                    elif type == 'mylif':
                        conv.append(DynamicReshapeModule(
                            LIF(n_in=channels * 32, n_out=channels * 32, t_len=T, t_latency=latency, recurrent=False,
                                beta_grad=True, adapt=False, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
                elif i == 1:
                    # 添加Blocks实例，设置参数
                    if type == 'blockalif':
                        conv.append(DynamicReshapeModule(
                            Blocks(n_in=channels * 16, n_out=channels * 16, t_len=T, t_latency=latency, recurrent=False,
                                   beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
                    elif type == 'alif':
                        conv.append(DynamicReshapeModule(
                            ALIF(n_in=channels * 16, n_out=channels * 16, t_len=T, t_latency=latency, recurrent=False,
                                 beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
                    elif type == 'lif':
                        conv.append(neuron.LIFNode(tau=2., surrogate_function=surrogate.ATan(), detach_reset=True,
                                                   step_mode='m', backend='cupy'))
                        ...
                        conv.append(DynamicReshapeModule(
                            LIF(n_in=channels * 16, n_out=channels * 16, t_len=T, t_latency=latency, recurrent=False,
                                beta_grad=True, adapt=False, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())))
        ...
        if type == 'blockalif':
            self.fc = nn.Sequential(
                ...
                DynamicReshapeModule(
                    Blocks(n_in=channels * 8 // 4, n_out=channels * 8 // 4, t_len=T, t_latency=latency, recurrent=False,
                           beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())),
            ...
            self.fc = nn.Sequential(
                ...
                layer.Linear(channels * 8, channels * 8 // 4),
                DynamicReshapeModule(
                    ALIF(n_in=channels * 8 // 4, n_out=channels * 8 // 4, t_len=T, t_latency=latency, recurrent=False,
                         beta_grad=True, adapt=True, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())),
                layer.Linear(channels * 8 // 4, 10),
            )
        elif type == 'lif':
            self.fc = nn.Sequential(
                layer.Flatten(),
                layer.Linear(channels * 8, channels * 8 // 4),
                neuron.LIFNode(tau=2., surrogate_function=surrogate.ATan(), detach_reset=True, step_mode='m',
                               backend='cupy'),
                layer.Linear(channels * 8 // 4, 10),
            )
        elif type == 'mylif':
            self.fc = nn.Sequential(
                ...
                layer.Linear(channels * 8, channels * 8 // 4),
                DynamicReshapeModule(
                    LIF(n_in=channels * 8 // 4, n_out=channels * 8 // 4, t_len=T, t_latency=latency, recurrent=False,
                        beta_grad=True, adapt=False, init_beta=0.99, init_p=0.99, surr_grad=surrogate.ATan())),
                layer.Linear(channels * 8 // 4, 10),
            )
...
def main():
    ...
    '''
    python blockalif_scf10_sg.py -data-dir /datasets/CIFAR10 -amp -opt sgd -channels 128 -epochs 256 -latency 1

    python blockalif_scf10.py -data-dir /datasets/CIFAR10 -amp -opt sgd -channels 128 -epochs 256 -sop -latency 1 -resume /home/wfang/chinese_review/scf10/logs/pt/logs/blockalifatan_la1_e256_b128_sgd_lr0.1_c128_amp/checkpoint_max.pth
    ...
    parser.add_argument('-prefix', default='blockalif_sg', type=str)
    ...
    net = CIFAR10Net(args.channels,32,type=args.type,latency = args.latency)
    net.to(args.device)
